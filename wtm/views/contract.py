...
def work_contract_reg(request, user_id):
    ...
    if request.method == 'POST':
        form = ContractForm(request.POST, branch=branch)
        if form.is_valid():
        ...
        form = ContractForm(branch=branch)

    # POST방식이지만 form에 오류가 있거나, GET방식일때 아래로 진행
    target_user = get_object_or_404(User, pk=user_id, branch=branch)  # 근로계약 대상 표시를 위함
    module_list = Module.objects.filter(branch=branch).order_by('order', 'id')  # 근로모듈을 요일별로 입력하기 위함
...
def work_contract_modify(request, contract_id):
    branch = request.user.branch
    contract = get_object_or_404(Contract, pk=contract_id, branch=branch)
    ...
    if request.method == 'POST':
        ...
        form = ContractForm(request.POST, instance=contract, branch=branch)
    ...
    # GET 방식으로 수정화면 호출
        ...
        form = ContractForm(instance=contract, branch=branch)

    # POST방식이지만 form에 오류가 있거나, GET방식일때 아래로 진행
    target_user = get_object_or_404(User, pk=user_id, branch=branch)  # 근로계약 대상 표시를 위함
    module_list = Module.objects.filter(branch=branch)  # 근로모듈을 요일별로 입력하기 위함
...
def work_contract_delete(request, contract_id):
    contract = get_object_or_404(Contract, pk=contract_id, branch=request.user.branch)
    user_id = contract.user_id
    # if request.user != question.author:
