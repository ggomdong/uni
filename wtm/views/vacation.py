...
def build_vacation_rows(year: int, *, is_contract_checked: bool = False) -> list[dict]:
    """
    ...
    vacation_module_ids = list(
        Module.objects.filter(name__iexact="연차").values_list("id", flat=True)
    )
    ...
    for month in range(1, 13):
        ...
        base_users = fetch_base_users_for_month(
            stand_ym, is_contract_checked=is_contract_checked
        )
        if not base_users:
            continue
...
def work_vacation_json(request, year: str | None = None):
    ...
    rows = build_vacation_rows(y, is_contract_checked=is_contract_checked)
